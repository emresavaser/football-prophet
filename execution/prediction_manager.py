...
class PredictionManager:
    ...
    async def predict_match(
        ...
        stored_prediction = await self._db.save_prediction(
            match_id=match_id,
        ...
        if value_bets:
            await self._db.save_value_bets(
    ...
    async def check_results(self) -> list[dict]:
        ...
        for pred in pending:
            ...
            await self._settle_value_bets(pred.id, match)
    ...
    async def _settle_value_bets(self, prediction_id: int, match) -> None:
        value_bets = await self._db.get_value_bets_for_prediction(prediction_id)
        if not value_bets:
            return

        latest_odds = await self._db.get_latest_odds(match.id)
        settled_at = datetime.now(timezone.utc)

        for value_bet in value_bets:
            closing_odds, closing_implied_prob, clv = self._compute_closing_line(value_bet, latest_odds)
            won, push, result = self._grade_value_bet(value_bet.market, match.home_score, match.away_score)
            pnl = 0.0 if push else ((value_bet.odds - 1.0) * value_bet.kelly_stake if won else -value_bet.kelly_stake)
            await self._db.settle_value_bet(
                value_bet.id,
                result=result,
                won=won,
                push=push,
    ...
    @staticmethod
    def _compute_closing_line(value_bet, latest_odds) -> tuple[Optional[float], Optional[float], Optional[float]]:
        odds_field = MARKET_ODDS_FIELDS.get(value_bet.market)
        ...
        clv = round((value_bet.odds - closing_odds) / closing_odds, 4) if closing_odds > 0 else None
        return closing_odds, closing_implied_prob, clv

    @staticmethod
    def _grade_value_bet(market: str, home_score: int, away_score: int) -> tuple[bool, bool, str]:
        total_goals = home_score + away_score
        if market == "home_win":
            return home_score > away_score, False, "WIN" if home_score > away_score else "LOSS"
        if market == "draw":
            return home_score == away_score, False, "WIN" if home_score == away_score else "LOSS"
        if market == "away_win":
            return away_score > home_score, False, "WIN" if away_score > home_score else "LOSS"
        if market == "over_25":
            return total_goals > 2.5, False, "WIN" if total_goals > 2.5 else "LOSS"
        if market == "under_25":
            return total_goals < 2.5, False, "WIN" if total_goals < 2.5 else "LOSS"
        both_teams_scored = home_score > 0 and away_score > 0
        if market == "btts_yes":
            return both_teams_scored, False, "WIN" if both_teams_scored else "LOSS"
        if market == "btts_no":
            return not both_teams_scored, False, "WIN" if not both_teams_scored else "LOSS"
        return False, False, "UNSETTLED"

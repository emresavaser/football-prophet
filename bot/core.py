...
class FootballProphet:
    ...
    async def initialize(self) -> None:
        ...
        self.backtest_engine = BacktestEngine(
            ...
            initial_model_weights=self.config.model_weights.as_dict(),
            elo_k_factor=self.config.elo_k_factor,
            form_window=self.config.form_window,
            form_decay=self.config.form_decay,
        )

        await self.train_models_from_history(self.config.active_leagues)
    ...
    async def train_models_from_history(self, league_codes: Optional[list[str]] = None) -> dict:
        """Train ensemble sub-models from finished historical matches in the DB."""
        league_codes = league_codes or self.config.active_leagues
        matches = []
        for league_code in league_codes:
            matches.extend(await self.db.get_league_matches(league_code, finished_only=True))

        summary = self.ensemble.train_ml_from_matches(matches)
        log.info("ml_training_summary", **summary)
        return summary

    ...
    async def _scan_matches(self) -> None:
        ...
        for match_info in upcoming:
            ...
            if p.odds_provider:
                ...
                cached = p.cache.get_odds(external_id) if external_id else None
                if not cached:
            ...
            try:
                ...
                for raw_odds in odds_list:
                    odds_payload = {
                        ...
                        "btts_yes": raw_odds.btts_yes,
                        "btts_no": raw_odds.btts_no,
                    }
                    p.cache.cache_odds(raw_odds.match_external_id, odds_payload)

                    match = await p.db.get_match_by_external_id(raw_odds.match_external_id)
                    if not match:
                        log.debug(
                            "odds_match_not_found",
                            external_id=raw_odds.match_external_id,
                            league=league_code,
                        )
                        continue

                    p.cache.cache_odds(match.id, odds_payload)
                    await p.db.save_odds(
                        match_id=match.id,
                        bookmaker=raw_odds.bookmaker,
                        home_win=raw_odds.home_win,
                        draw=raw_odds.draw,
                        away_win=raw_odds.away_win,
                        over_25=raw_odds.over_25,
